...
-- Enable UUID extension (if needed, but we'll use BIGSERIAL for IDs)
...
CREATE TABLE IF NOT EXISTS delivery_boys (
    ...
    store_id BIGINT REFERENCES users(id) ON DELETE SET NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
BEGIN
    ...
    SELECT COALESCE(
        MAX(
            CASE 
                WHEN order_number ~ ('^ORD-' || store_id_param || '-(\d+)$')
                THEN CAST(SUBSTRING(order_number FROM ('ORD-' || store_id_param || '-(\d+)$')) AS INTEGER)
                ELSE 0
            END
        ), 0
    ) + 1
    ...
    order_num := 'ORD-' || store_id_param || '-' || LPAD(sequence_num::TEXT, 3, '0');
