...
export default function IdleTimeoutWarning({
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            if (timeRemaining > 0) {
                const newTimeRemaining = timeRemaining - (1 / 60);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
            <div className="bg-white rounded-lg shadow-xl p-6 max-w-md w-full mx-4 border-2 border-yellow-400">
                ...
                <div className="flex gap-3">
                    <button
                        onClick={onExtend}
                        className="flex-1 bg-blue-600 text-white px-4 py-2.5 text-sm font-medium rounded-lg hover:bg-blue-700 transition-all shadow-md"
                    >
                        Stay Logged In
                    </button>
                    <button
