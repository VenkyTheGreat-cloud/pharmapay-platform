...
const login = async () => {
  ...
  const response2 = await fetch('https://pharmapay-api.onrender.com/api/auth/login', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
...
### Test Registration
```bash
curl -X POST https://pharmapay-api.onrender.com/api/auth/register \
...
```bash
curl -X POST https://pharmapay-api.onrender.com/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{
