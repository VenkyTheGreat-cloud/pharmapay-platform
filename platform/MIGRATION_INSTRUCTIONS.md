...
```bash
pg_dump -U postgres -d pharmapay > backup_before_migration.sql
```
...
```bash
psql -U postgres -d pharmapay -f scripts/check-data-before-migration.sql
```
...
```bash
psql -U postgres -d pharmapay -f scripts/migrate-foreign-keys-to-uuid-simple.sql
```
...
```bash
psql -U postgres -d pharmapay -f scripts/migrate-foreign-keys-to-uuid-safe.sql
```
...
```bash
psql -U postgres -d pharmapay < backup_before_migration.sql
```
