...
const changePassword = async (oldPassword, newPassword) => {
  const response = await fetch('https://sbb-medicare-api.onrender.com/api/auth/change-password', {
    method: 'POST',
    headers: {
...
```bash
curl -X POST https://sbb-medicare-api.onrender.com/api/auth/change-password \
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        'https://sbb-medicare-api.onrender.com/api/auth/change-password',
        {
