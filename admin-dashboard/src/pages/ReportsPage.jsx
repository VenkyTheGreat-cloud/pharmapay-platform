...
export default function ReportsPage() {
    ...
    const loadDeliveryBoys = async () => {
        try {
            ...
            const response = await deliveryBoysAPI.getAll({ status: 'approved' });
                ...
                case 'orders':
                    if (fromDate === toDate) {
                        response = await reportsAPI.exportOrdersReport({
                            date: fromDate,
                            from_time: fromTime.substring(0, 5),
                            to_time: toTime.substring(0, 5),
                        });
                    } else {
                        response = await reportsAPI.exportOrdersReport({
                            date_from: fromDate,
                    ...
                    <div className="space-y-6">
                                    ...
                                    <div className="space-y-3">
                                        <div>
                                            ...
                                            <input
                                                ...
                                                max={getTodayIST()}
                                                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-xs focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                                            />
                                        </div>
                                        <div>
                                            <label className="block text-xs font-medium text-gray-600 mb-2">
                        ...
                        <div className="flex justify-end pt-2">
                            <button
                                ...
                                className="bg-gradient-to-r from-primary-500 to-primary-600 text-white px-6 py-2 text-xs font-medium rounded-lg hover:from-primary-600 hover:to-primary-700 transition-all shadow-md flex items-center gap-2 disabled:opacity-50"
                            >
                                {isDownloading ? (
                                    <>
